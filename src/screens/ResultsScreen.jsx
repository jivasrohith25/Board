...
export function ResultsScreen() {
  ...
  const podiumGradients = {
    1: 'bg-gradient-to-t from-primary-600 to-primary-400',
    2: 'bg-gradient-to-t from-primary-300 to-primary-200',
    3: 'bg-gradient-to-t from-warm-200 to-warm-100',
  }
  ...
  return (
    <div className="min-h-screen bg-warm-50 relative overflow-hidden">
      ...
      <div className="relative z-10 px-4 py-8 max-w-lg mx-auto">
        ...
        >
          <h1 className="font-display text-display-lg text-warm-900 mb-1">🏆 Game Over!</h1>
          <p className="text-warm-400 text-sm">Here are the final standings</p>
        </motion.div>
            ...
            return (
              ...
              >
                <div className="mb-2 text-center">
                  {/* Rank badge */}
                  <motion.div
                    className={`${player.rank === 1 ? 'text-4xl' : 'text-2xl'} mb-1`}
                    animate={player.rank === 1 ? { y: [0, -4, 0], rotate: [0, 4, -4, 0] } : {}}
                  ...
                  <p className="font-display font-bold text-warm-900 text-sm truncate max-w-[110px] mt-1.5">
                    {player.name}
                  </p>
                  {isTied && (
                    <span className="inline-block text-[9px] font-bold text-primary-500 bg-primary-100 px-1.5 py-0.5 rounded-full mb-0.5">
                      TIED
                    </span>
                  )}
                  <p className="font-mono font-extrabold text-lg text-primary-600 tabular-nums">
                    {player.score}
                  </p>
                </div>
                <motion.div
                  className={`w-full ${podiumHeights[player.rank]} ${podiumGradients[player.rank]} rounded-t-xl ${
                    player.rank === 1 ? 'shadow-lg shadow-primary-300/30' : ''
                  }`}
                  initial={{ height: 0 }}
                  animate={{
                    height: 'auto',
                    boxShadow: player.rank === 1
                      ? ['0 0 0px 0px rgba(237,128,39,0)', '0 0 16px 3px rgba(237,128,39,0.25)', '0 0 0px 0px rgba(237,128,39,0)']
                      : undefined,
                  }}
                  transition={{
                    height: { delay: podiumDelay[player.rank] + 0.1, duration: 0.5, ease: 'easeOut' },
                    boxShadow: player.rank === 1
                      ? { delay: 1, duration: 2.5, repeat: Infinity, ease: 'easeInOut' }
                      : undefined,
                  }}
                />
              </motion.div>
        ...
        {rest.length > 0 && (
          ...
          >
            <h3 className="section-label mb-2.5">Other Players</h3>
            <div className="space-y-1.5">
              {rest.map((player, i) => {
                const isTied = i > 0 && player.score === rest[i - 1].score
                return (
                  <div key={player.name} className="flex items-center gap-3 py-1.5 px-1">
                    <span className="w-7 text-center text-warm-300 font-bold text-xs font-mono">
                      #{i + 4}
                    </span>
                    <PlayerAvatar name={player.name} avatar={player.avatar} size="sm" />
                    <span className="flex-1 font-medium text-warm-700 text-sm">
                      {player.name}
                      {isTied && (
                        <span className="ml-1.5 text-[9px] font-bold text-primary-500 bg-primary-100 px-1.5 py-0.5 rounded-full">
                          TIE
                        </span>
                      )}
                    </span>
                    <span className="font-mono font-bold text-warm-600 text-sm tabular-nums">{player.score}</span>
                  </div>
                )
              })}
            </div>
        ...
        >
          <h3 className="section-label mb-3">📊 Score Overview</h3>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="text-[11px] font-semibold text-warm-700 truncate">{player.name}</span>
                      <span className="text-[11px] font-mono font-bold text-primary-600 ml-2 tabular-nums">{player.score}</span>
                    </div>
        ...
        {analytics && (
          ...
          >
            <h3 className="section-label mb-3">📊 Game Analytics</h3>
            <div className="grid grid-cols-2 gap-2 mb-4">
              <div className="bg-warm-50 rounded-xl p-3 text-center border border-warm-100/60">
                <p className="stat-value">{analytics.numRounds}</p>
                <p className="text-[10px] text-warm-400 font-medium uppercase tracking-wide mt-0.5">Rounds</p>
              </div>
              <div className="bg-warm-50 rounded-xl p-3 text-center border border-warm-100/60">
                <p className="stat-value">{analytics.totalPoints}</p>
                <p className="text-[10px] text-warm-400 font-medium uppercase tracking-wide mt-0.5">Total Points</p>
              </div>
              <div className="bg-warm-50 rounded-xl p-3 text-center border border-warm-100/60">
                <p className="stat-value">{analytics.avgScore}</p>
                <p className="text-[10px] text-warm-400 font-medium uppercase tracking-wide mt-0.5">Avg / Player</p>
              </div>
              <div className="bg-warm-50 rounded-xl p-3 text-center border border-warm-100/60">
                <p className="stat-value">{analytics.margin}</p>
                <p className="text-[10px] text-warm-400 font-medium uppercase tracking-wide mt-0.5">Win Margin</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {analytics.highestRoundScore > 0 && (
                <div className="flex items-start gap-2 text-sm">
                  ...
                  <span className="text-warm-600 leading-snug">
                    <span className="font-bold text-warm-800">{analytics.highestRoundPlayer}</span> scored{' '}
              ...
              {analytics.maxComeback > 0 && analytics.comebackPlayer && (
                <div className="flex items-start gap-2 text-sm">
                  <span className="text-base mt-px">💪</span>
                  <span className="text-warm-600 leading-snug">
                    <span className="font-bold text-warm-800">{analytics.comebackPlayer}</span> overcame a{' '}
              ...
              {analytics.playerConsistency.length > 0 && (
                <div className="flex items-start gap-2 text-sm">
                  <span className="text-base mt-px">🎯</span>
                  <span className="text-warm-600 leading-snug">
                    Most consistent: <span className="font-bold text-warm-800">{analytics.playerConsistency[0].name}</span>
            ...
            {analytics.playerConsistency.length > 0 && (
              <div className="mt-3 pt-3 border-t border-warm-100">
                <p className="section-label mb-2">Player Averages</p>
                <div className="space-y-1.5">
                  {analytics.playerConsistency.map((p, i) => (
                    <div key={p.name} className="flex items-center gap-2.5 py-0.5">
                      <span className="w-5 text-center text-warm-300 font-bold text-[10px] font-mono">#{i + 1}</span>
                      <PlayerAvatar name={p.name} avatar={p.avatar} size="xs" />
                      <span className="flex-1 font-medium text-warm-700 text-xs">{p.name}</span>
                      <span className="font-mono text-warm-500 text-[11px] tabular-nums">avg {p.avg}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        >
          <div className="flex items-center justify-center gap-2.5">
            <motion.button
              ...
              className="btn-ghost border border-warm-200/60 px-5 text-sm"
            >
              🏠 Home
            </motion.button>
            ...
            {archivedGameId && !archiving && (
              <motion.button
                ...
                className="btn-ghost border border-warm-200/60 px-4 text-sm"
              >
                📜 History
              </motion.button>
            )}
            {archiving && (
              <div className="w-9 h-9 rounded-full bg-warm-100 flex items-center justify-center">
                <svg className="w-4 h-4 text-warm-400 animate-spin" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
                </svg>
              </div>
